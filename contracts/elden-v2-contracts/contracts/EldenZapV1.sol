...
import {IEldenPair} from "./interfaces/IEldenPair.sol";
import {IEldenRouter02} from "./interfaces/IEldenRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by EldenSwap teams.
 */
contract EldenZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // EldenRouter interface
    IEldenRouter02 public eldenRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from EldenSwap)
    ...
    // Address EldenRouter
    address private eldenRouterAddress;
     ...
     * @param _eldenRouter: address of the EldenRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WETHAddress,
        address _eldenRouter,
        uint256 _maxZapReverseRatio
    ) {
        WETHAddress = _WETHAddress;
        WETH = IWETH(_WETHAddress);
        eldenRouterAddress = _eldenRouter;
        eldenRouter = IEldenRouter02(_eldenRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. ELDEN/ETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ELDEN) to receive in the intermediary swap (e.g. ETH --> ELDEN)
     */
     ...
     * @param _lpToken: LP token address (e.g. ELDEN/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ELDEN) in the intermediary swap (e.g. BUSD --> ELDEN)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. ELDEN/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. ELDEN --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. ELDEN/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ELDEN or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ELDEN) in the intermediary swap (e.g. BUSD --> ELDEN)
     */
    ...
    {
        address token0 = IEldenPair(_lpToken).token0();
        address token1 = IEldenPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IEldenPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = eldenRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = eldenRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IEldenPair(_lpToken).token0() || _token0ToZap == IEldenPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IEldenPair(_lpToken).token0() || _token1ToZap == IEldenPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IEldenPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IEldenPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IEldenPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = eldenRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = eldenRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IEldenPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "EldenRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "EldenRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        eldenRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), eldenRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(eldenRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = eldenRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = eldenRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = eldenRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = eldenRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = eldenRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
