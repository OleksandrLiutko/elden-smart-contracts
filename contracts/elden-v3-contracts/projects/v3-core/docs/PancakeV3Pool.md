...
The contract that deployed the pool, which must adhere to the IEldenV3Factory interface
...
```solidity
struct EldenV3Pool.Slot0 slot0
```
...
```solidity
struct EldenV3Pool.ProtocolFees protocolFees
```
...
```solidity
contract IEldenV3LmPool lmPool
```
...
_Prevents calling a function from anyone except the address returned by IEldenV3Factory#owner()_
...
_The caller of this method receives a callback in the form of IEldenV3SwapCallback#eldenV3SwapCallback_
...
_The caller of this method receives a callback in the form of IEldenV3FlashCallback#eldenV3FlashCallback
...
```solidity
function setLmPool(contract IEldenV3LmPool _lmPool) external
```
