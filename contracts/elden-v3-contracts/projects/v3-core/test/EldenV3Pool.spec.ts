...
import { TestEldenV3SwapPay } from '../typechain-types/contracts/test/TestEldenV3SwapPay'
...
import { TestEldenV3ReentrantCallee } from '../typechain-types/contracts/test/TestEldenV3ReentrantCallee'
...
describe('EldenV3Pool', () => {
  ...
  let pool: MockTimeEldenV3Pool

  let swapTarget: TestEldenV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeEldenV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestEldenV3ReentrantCallee')
      ).deploy()) as TestEldenV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestEldenV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestEldenV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestEldenV3SwapPay
