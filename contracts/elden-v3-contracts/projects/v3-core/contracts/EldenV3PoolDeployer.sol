...
contract EldenV3PoolDeployer is IEldenV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IEldenV3PoolDeployer
    ...
    function deploy(
        ...
        pool = address(new EldenV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
