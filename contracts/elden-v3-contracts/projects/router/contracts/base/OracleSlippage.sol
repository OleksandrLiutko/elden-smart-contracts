...
import '@elden/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@elden/v3-periphery/contracts/libraries/Path.sol';
import '@elden/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@elden/v3-core/contracts/interfaces/IEldenV3Pool.sol';
import '@elden/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IEldenV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IEldenV3Pool pool) {
        pool = IEldenV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IEldenV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IEldenV3Pool(pool).slot0();
            }
