...
| Name | Type | Description |
| ---- | ---- | ----------- |
| deployer | address | The EldenSwap V3 deployer contract address |
...
```solidity
function getPool(address deployer, address tokenA, address tokenB, uint24 fee) public pure returns (contract IEldenV3Pool)
```
...
```solidity
function verifyCallback(address deployer, address tokenA, address tokenB, uint24 fee) public view returns (contract IEldenV3Pool pool)
```
...
```solidity
function verifyCallback(address deployer, struct SmartRouterHelper.PoolKey poolKey) public view returns (contract IEldenV3Pool pool)
```
