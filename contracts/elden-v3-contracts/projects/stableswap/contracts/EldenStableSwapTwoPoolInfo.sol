...
contract EldenStableSwapTwoPoolInfo {
    ...
    function calc_coins_amount(address _swap, uint256 _amount) external view returns (uint256[N_COINS] memory) {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            uint256 value = (IEldenStableSwap(_swap).balances(i) * _amount) / total_supply;
            amounts[i] = value;
        }
        return amounts;
    }
    ...
    function _xp(address _swap) internal view returns (uint256[N_COINS] memory result) {
        result = RATES(_swap);
        for (uint256 i = 0; i < N_COINS; i++) {
            result[i] = (result[i] * IEldenStableSwap(_swap).balances(i)) / PRECISION;
        }
    }
