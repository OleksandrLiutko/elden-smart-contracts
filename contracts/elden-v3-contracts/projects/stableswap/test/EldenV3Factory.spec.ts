...
describe('EldenV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: EldenV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as EldenV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as EldenV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [EldenV3Factory, EldenV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('EldenV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('EldenV3Pool')
